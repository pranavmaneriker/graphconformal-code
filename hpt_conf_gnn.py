...
import ray.train
from graph_conformal.config import ConfExptConfig, DatasetSplitConfig
from graph_conformal.constants import ConformalMethod, Stage, layer_types, sample_type
...
def create_tune_jobid_from_config(config: ConfExptConfig) -> str:
    ...
    match loading_style:
        case sample_type.split.name:
            ...
            return f"{config.dataset}_{config.confgnn_config.model}_{loading_style}_{split_fractions.train}_{split_fractions.valid}"  # type: ignore
        case sample_type.n_samples_per_class.name:
            return f"{config.dataset}_{config.confgnn_config.model}_{loading_style}_{config.dataset_n_samples_per_class}"  # type: ignore
        case _:
...
def main():
    ...
    args.conf_expt_config.dataset = args.dataset
    ...
    for split_config in expt_loop_space:
        ...
        search_space = {
            ...
            f"{CFGNN_PREFIX}hidden_channels": tune.choice([16, 32, 64, 128]),  # 256
        ...
        match l_type:
            case layer_types.GAT.name:
                search_space[f"{CFGNN_PREFIX}heads"] = tune.choice([2, 4])  # 8
